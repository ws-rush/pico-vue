...
This directive is only needed in no-build-step setups. When using pico-vue directly in the browser, there might be a brief moment where the raw template (e.g. `{{ message }}`) is visible before pico-vue compiles it.
