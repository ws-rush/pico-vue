...
**If your page renders user-submitted HTML, you should prefer initializing pico-vue using [explicit mount target](/essentials/v-scope#explicit-mount-target) so that it only processes parts that are controlled by you**.

You can also sanitize any user-submitted HTML for the `v-scope` attribute.

## Content Security Policy (CSP)

pico-vue evaluates expressions using `new Function()`, which may be prohibited in strict CSP settings.
